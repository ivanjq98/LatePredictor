  ...
  if (submitted && pollResult) {
    return (
      ...
      >
        ...
        >
          {pollResult.totalVotes} vote{pollResult.totalVotes !== 1 ? "s" : ""}{" "}
          cast
        </div>
        {pollResult.winners.slice(0, 3).map((w, i) => (
          ...
          >
            {["🥇", "🥈", "🥉"][i]} <strong>{w.username}</strong> +{w.points}{" "}
            pts
          </div>
        ))}
      </div>
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          textTransform: "uppercase" as const,
          fontFamily: "sans-serif",
          cursor: actual !== null ? "pointer" : "not-allowed",
          transition: "all 0.2s",
        }}
      >
        {loading
