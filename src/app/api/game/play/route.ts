...
const roles = ["Blossom", "Bubbles", "Buttercup", "Mojo Jojo"].sort(
  () => Math.random() - 0.5,
);

const ASSETS = {
  LOBBY:
    "https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExeHdxZm53NTFwaHBjaDM1NDh6Y2EzcDQ2aWY3NXE1anZpaGNkeWFsZyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/BD3jBIALsbOI8/giphy.gif", // PPG flying
  MOJO_WIN:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExcDkydXBmdnR2Y3Y1bnA4MmQxcWNzbjB2bWg0YWF5MDEzcG05a2YwcyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/9dcxwnY5Fl65xW6mjl/giphy.gif", // Mojo laughing
  GIRLS_WIN:
    "https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExMXdlaG91ZG9hYm0yMTRyanF0N2lmcDA0cGNha250aGdxNzNzOXRtciZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/KpDnCsHaiO7o4/giphy.gif", // Girls celebrating
  WRONG_VOTE:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExNW1scmxwNGpsY3pldzY0YnVlY2JzMms4eGozamtwYnM5NHpxcmM3dyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/136NDK9Pzd4WWLeHvg/giphy.gif", // Buttercup being annoyed
  ...
  BUBBLES:
    "https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExNXNvbDFpeWt1YndiajA3ZGRybHdpNTFxNGwybWFxMmxwbTZiMTFodSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/k3UkjmH76JtE4H6ZOR/giphy.gif",
  BLOOSOM:
    "https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExdWQ4Ymg1Z3l6d2xqejFmbGNoNDljNmY3MzZkcHpkOWYxYWNiNTU4NiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/Qumb7wwbzIQVxUuWw9/giphy.gif",
  BUTTERCUP:
    "https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExMTZlZDhvdDd0MDV2amx4cDNyMGZ1dDMxdmdkM29zNG44bXd2Nnd5MCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/bu3OZCHFpjzk4/giphy.gif",
};

async function tgSendVisual(
  method: "sendAnimation" | "sendPhoto",
  chatId: number,
  url: string,
  caption: string,
) {
...
async function fetchRandomTracks(count: number = 10): Promise<DeezerTrack[]> {
  ...
  const res = await fetch(
    `https://api.deezer.com/search?q=${encodeURIComponent(term)}&limit=25&index=${offset}`,
  );
  const data = await res.json();
  const tracks: DeezerTrack[] = (data.data || []).filter(
    (t: DeezerTrack) => t.preview && t.title && t.artist?.name,
  );
...
async function sendNewRound(chatId: number, gameMode: string): Promise<void> {
  ...
  if (tracks.length < 4) {
    await sendTelegramMessage(
      chatId,
      "⚠️ Couldn't fetch songs right now. Please try again.",
    );
    return;
  }
  ...
  const correctAnswer =
    gameMode === "artist" ? correctTrack.artist.name : correctTrack.title;
  ...
  const options = [correctAnswer, ...uniqueWrong].sort(
    () => Math.random() - 0.5,
  );
  ...
  const buttons = [
    [
      { text: options[0], callback_data: "answer_0" },
      { text: options[1], callback_data: "answer_1" },
    ],
    [
      { text: options[2], callback_data: "answer_2" },
      { text: options[3], callback_data: "answer_3" },
    ],
  ];
  ...
  try {
    ...
    if (body.poll_answer) {
      ...
      if (poll) {
        const { error: insertError } = await supabase.from("poll_votes").upsert(
          {
            ...
            username:
              body.poll_answer.user.username ||
              body.poll_answer.user.first_name ||
              "Anonymous",
        ...
        console.log(
          "3. No poll found in DB with ID:",
          body.poll_answer.poll_id,
        );
      }
    ...
    if (isButton && body.callback_query.data === "ppg_join") {
      ...
      await supabase
        .from("ppg_players")
        .insert({ chat_id: chatId, user_id: userId, username });
      ...
      if (count < 4) {
        await tgPost("sendMessage", {
          chat_id: chatId,
          text: `🎮 ${username} joined! (${count}/4 players)`,
        });
      } else {
        ...
        for (let i = 0; i < players!.length; i++) {
          ...
          await supabase
            .from("ppg_players")
            .update({ role: assignedRole, is_mojo: isMojo })
          ...
          if (assignedRole == "Blossom")
            ...
            await tgSendVisual(
              ...
              p.user_id,
              ASSETS.MOJO,
              "Mojo Jojo, with a plan to take the world!",
            );
        }

        const voteButtons = players!.map((p) => [
          {
            text: `Vote for ${p.username}`,
            callback_data: `ppg_vote_${p.user_id}`,
          },
        ]);
    ...
    if (isButton && body.callback_query.data.startsWith("ppg_vote_")) {
      ...
      if (totalVoted === alivePlayers?.length) {
        ...
        const expelledId = Object.keys(voteCounts).reduce((a, b) =>
          voteCounts[a] > voteCounts[b] ? a : b,
        );
        const expelledUser = alivePlayers?.find(
          (p) => p.user_id.toString() === expelledId,
        );
        ...
        const deathReason =
          funnyDeaths[Math.floor(Math.random() * funnyDeaths.length)];
        ...
        **The End:** He was last seen being chased out of Townsville by a very angry squirrel. 🐿️
          ...
          await tgSendVisual(
            ...
            chatId,
            ASSETS.GIRLS_WIN,
            "🎉 *VICTORY!* Mojo Jojo has been defeated! The Powerpuff Girls save the day",
          );
          await supabase.from("ppg_players").delete().eq("chat_id", chatId);
        } else {
          ...
          const girlCount =
            remainingPlayers?.filter((p) => !p.is_mojo).length || 0;
        ...
        **The End:** Mojo Jojo has successfully renamed Townsville to "Mojo-opolis" and declared that every day is now "Wear a Funny Hat" day. 🎩
        ...
        ❌ *GIRLS LOSE!*
            ...
            await tgSendVisual(
              ...
              chatId,
              ASSETS.MOJO_WIN,
              "🐵 *MOJO JOJO WINS!* All your Chemical X are belong to me!",
            );
            await supabase.from("ppg_players").delete().eq("chat_id", chatId); // Reset DB for next game
          } else {
            // --- CONTINUE TO NEXT ROUND ---
            const nextButtons = remainingPlayers!.map((p) => [
              {
                text: `Suspect: ${p.username}`,
                callback_data: `ppg_vote_${p.user_id}`,
              },
            ]);
    ...
    if (text === "/start_quiz") {
      ...
      await sendTelegramMessage(chatId, `🎧 *Music Quiz!* Choose your mode:`, [
        [
          { text: "🎵 Song", callback_data: "mode_song" },
          { text: "🎤 Artist", callback_data: "mode_artist" },
        ],
      ]);
      return NextResponse.json({ ok: true });
    }
    ...
    if (text === "/end_quiz") {
      ...
      if (game) {
        await sendTelegramMessage(
          chatId,
          "You have quit the music pop quiz 🙂‍↔️",
        );
      }
      ...
      if (!game) {
        await sendTelegramMessage(
          chatId,
          "⚠️ You don't have an active quiz running.",
        );
        return NextResponse.json({ ok: true });
      }
    ...
    if (isButtonPress) {
      ...
      if (callbackData.startsWith("guess_")) {
        ...
        if (!game) {
          await sendTelegramMessage(
            chatId,
            "⚠️ No active Number Game. Type /start_game",
          );
          return NextResponse.json({ ok: true });
        }
        ...
        if (guess === game.secret_number) {
          ...
          await sendTelegramMessage(
            chatId,
            `🎉 *CORRECT!* It was ${game.secret_number}! Game Over.`,
          );
        } else {
          await supabase
            .from("number_games")
            .update({ attempts: (game.attempts || 0) + 1 })
            .eq("user_id", chatId);
      ...
      if (callbackData.startsWith("answer_")) {
        ...
        if (!game) {
          await sendTelegramMessage(
            chatId,
            "⚠️ No active Music Quiz. Type /start_quiz",
          );
          return NextResponse.json({ ok: true });
        }
        ...
        if (isCorrect) {
          ...
          await sendTelegramMessage(
            chatId,
            `❌ *Wrong!* It was: ${game.correct_answer}`,
          );
        }
